...
const API_BASE_STORAGE_KEY = 'posApiBaseUrl';
...
function getOfflineApiMessage() {
  return 'No hay backend API disponible. Si estás en GitHub Pages, configura tu backend y guarda su URL en localStorage["posApiBaseUrl"] (ej: https://tu-backend.com).';
}
...
async function api(path, method='GET', body){
  ...
  try {
    ...
    throw new Error(getOfflineApiMessage());
  }
  ...
  if (!res.ok) {
    const msg = isJson ? payload?.error : null;
    throw new Error(msg || getOfflineApiMessage());
  }
